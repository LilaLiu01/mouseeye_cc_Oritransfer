...
rawdataall <- read_excel("AllData_exp3.xlsx", sheet="Exp3RawData")
setDT(rawdataall)
...
Overall accuracy is high for both configuration across epochs. The repeated ANOVA on accuracies indicates no potential speed-accuracy trade-offs regarding configuration type. 
...
(line_plot_se_testing <- ggplot(dfwc_testing, aes(x = as.factor(Block), y = RT * 1000, color = Configuration)) +
  ...
  labs(x = "Testing Block", y = "RT (ms)", title = "Exp 3") +
...
ggsave("Exp3RT_Testing.jpg", line_plot_se_testing + theme(plot.background = element_rect(fill = "white")))
...
ggsave("Exp3RT.png", plot + theme_minimal() + theme(plot.background = element_rect(fill = "white")))
```
...
# RT by Config across blocks(correct only)
tempdata <- as.data.table(tempdata)
write.csv(tempdata, file = "RT_exp3.csv")
...
write.csv(RTexp, file = "Exp3AData.csv")
...
ggsave("Exp3Slopes.png", plot + theme(plot.background = element_rect(fill = "white")))
```
## Correlation between RT saving in training phase and d'
...
## Correlation between RT saving and d' in testing phase
...
RT_norm_exp <- dfwc %>% 
  ...
  theme_publish() +
  geom_vline(xintercept = 5.4, linetype="dotted") +
  annotate("text", x=3, y=0.75, label= "training\n phase", size=15/.pt) + 
  annotate("text", x=6, y=0.75, label= "testing\n phase", size=15/.pt) +
  labs(color = "Configuration", shape = "Configuration")+
  labs(title = "Exp 3") +
  theme(legend.position="top",
        aspect.ratio = 0.8)+
...
ggsave("Exp3RTStandard.png", plot + theme_minimal() + theme(plot.background = element_rect(fill = "white")))
```
...
# RT by Config across blocks(correct only)
tempdata <- as.data.table(tempdata)
write.csv(tempdata, file = "RTnorm_exp3.csv")
RTexp <- tempdata[filter==1,
                     .(RT = mean(RT)),
                     keyby = .(Sub,Phase,Configuration,Block)]
RTexp$Sub <- as.factor(RTexp$Sub)
...
RTexp <- RTexp[, .(RT = mean(RT)),
                     keyby = .(Sub,Configuration,Epoch)]
write.csv(RTexp, file = "Exp3AZscoreData.csv")
...
RTlong_exp3 <- read.csv("RT_exp3.csv")
RTnorm_exp3 <- read.csv("RTnorm_exp3.csv")


RTlong_exp3 <- RTlong_exp3 %>%
  mutate(Condition = "tunnel")
RTnorm_exp3 <- RTnorm_exp3 %>%
  mutate(Condition = "tunnel")
...
ggsave("Exp3Indi.png", plot + theme(plot.background = element_rect(fill = "white")))      
